...
app.get("/home", RequiredUserMiddleware, function(req, res) {
    res.render(getLocalePath("homePage.jsembeds", req.headers["accept-language"]), { username: he.encode(req.user.username), usernam_js: JSON.stringify(req.user.username) });
});
...
app.get("/api/friendTokens", RequiredUserMiddleware, function(req, res) {
    ...
    for (let friendToken in friendTokens) if (friendTokens[friendToken].to == req.user.username) filteredFriendTokens[friendToken] = friendTokens[friendToken];
...
app.get("/manageAccount/removeAccount", RequiredUserMiddleware, function(req, res) {
    ...
    for (let friend of req.user.object.friends) {
        let friendObj = user.getUserByName(friend) || {friends:[],notExist:true};
    ...
    for (let message of req.user.object.messages) {
        let contactObj = user.getUserByName(message.username) || {messages:[],notExist:true};
